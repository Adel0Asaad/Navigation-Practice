...
const DetailsScreen = ({ route, navigation }: DetailsProps) => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: myMovie.title, headerTransparent: true });
      ...
      <ScrollView style={{ flex: 1 }}>
        ...
        <View style={styles.horizontalSpaceContainer}>
          <View style={styles.titleContainer}>
            <Text style={styles.titleText}>{myMovie.title}</Text>
          </View>
          <View style={styles.voteContainer}>
            <Text style={styles.voteText}>
              {myMovie.vote_average.toFixed(1)}
            </Text>
            <Ionicons
              name="star"
        ...
        <Text style={styles.overviewText}>{myMovie.overview}</Text>
